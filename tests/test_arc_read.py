"""
Tests for scripts/arc-read.sh edge cases.

Verifies jq queries handle: empty projects, all-done items, malformed JSONL,
missing .arc/, items with/without tactical steps, waiting_for, standalone
outcomes, and mixed states.

These are the mechanical edge cases identified when arc-read.sh was written
...
ARC_READ = REPO_ROOT / "scripts" / "arc-read.sh"


def run_arc_read(tmp_path: Path, jsonl_content: str, command: str) -> subprocess.CompletedProcess:
    """Set up temp .arc/ directory and run arc-read.sh with given command."""
    arc_dir = tmp_path / ".arc"
    arc_dir.mkdir(exist_ok=True)
    (arc_dir / "items.jsonl").write_text(jsonl_content)
    return subprocess.run(
        [str(ARC_READ), command],
        capture_output=True,
        text=True,
        cwd=tmp_path,
    )
...
class TestNoArcDirectory:
    """When .arc/ doesn't exist, script exits silently."""
...
class TestEmptyJsonl:
    ...
    def test_empty_file_no_output(self, tmp_path, command):
        result = run_arc_read(tmp_path, "", command)
        assert result.returncode == 0
...
class TestAllDoneItems:
    ...
    def test_list_empty(self, tmp_path):
        ...
        result = run_arc_read(tmp_path, data, "list")
        assert result.returncode == 0
        assert result.stdout.strip() == ""
    ...
    def test_ready_empty(self, tmp_path):
        ...
        result = run_arc_read(tmp_path, data, "ready")
        assert result.returncode == 0
        assert result.stdout.strip() == ""
    ...
    def test_current_empty(self, tmp_path):
        ...
        result = run_arc_read(tmp_path, data, "current")
        assert result.returncode == 0
        assert result.stdout.strip() == ""
...
class TestUsageErrors:
    ...
    def test_unknown_command(self, tmp_path):
        result = run_arc_read(tmp_path, make_jsonl(OPEN_OUTCOME), "bogus")
    ...
    def test_no_command(self, tmp_path):
        arc_dir = tmp_path / ".arc"
        arc_dir.mkdir()
        (arc_dir / "items.jsonl").write_text(make_jsonl(OPEN_OUTCOME))
        result = subprocess.run(
            [str(ARC_READ)],
            capture_output=True,
            text=True,
