...
### Phase 0.5: Cross-machine working-tree sweep

Before the GitHub-side audit, sweep every local clone — across all machines — for work that exists only on disk (uncommitted) or only on one host (unpushed). Highest value, lowest effort: catches the "lost the laptop" and "two machines drifted" risks the remote audit can't see.

```bash
S=skills/github-cleanup/scripts/repo-sweep.sh   # from trousse root
"$S" local ~/Repos ~/.claude                    # local roots (fetches for true ahead/behind)
"$S" ssh hezza '$HOME/Repos' '$HOME/.claude'    # a remote host — same engine, runs there
```

The engine fetches by default — without it, ahead/behind reads against each machine's last fetch and a repo can look "ahead" from a stale ref alone. See [references/cross-machine-sweep.md](references/cross-machine-sweep.md) for flag meanings, triage rules, the stale-ref story, and the plugin version-bump guard.
