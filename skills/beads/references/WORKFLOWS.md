...
```
Session Start (with MCP - local environment):
- [ ] Use mcp__plugin_beads_beads__ready to find unblocked work
- [ ] Report: "X items ready to work on: [summary]"
- [ ] If using global ~/.beads, note this in report
- [ ] If none ready, use mcp__plugin_beads_beads__blocked to check blockers
...
**Pattern**: Always check for ready work when starting session where bd is available. Report status immediately to establish shared context.
...
```
After Compaction (with MCP):
- [ ] Use mcp__plugin_beads_beads__list with status:"in_progress" to see active work
- [ ] Use mcp__plugin_beads_beads__show for each in_progress issue
- [ ] Read notes field to understand: COMPLETED, IN PROGRESS, BLOCKERS, KEY DECISIONS
- [ ] Check dependencies via issue details
- [ ] If notes insufficient, list open issues to find related work
- [ ] Reconstruct TodoWrite list from notes if needed

After Compaction (CLI - web environment):
- [ ] Run bd list --status in_progress --json
- [ ] Run bd show <issue-id> --json for each in_progress issue
- [ ] Read notes field, check bd dep tree <issue-id> for context
```
...
Use `mcp__plugin_beads_beads__update` with detailed notes:
```json
{
  "issue_id": "issue-42",
  "notes": "COMPLETED: User authentication - added JWT token generation with 1hr expiry, implemented refresh token endpoint using rotating tokens pattern. IN PROGRESS: Password reset flow. Email service integration working. NEXT: Need to add rate limiting to reset endpoint (currently unlimited requests). KEY DECISION: Using bcrypt with 12 rounds after reviewing OWASP recommendations, tech lead concerned about response time but benchmarks show <100ms."
}
```

(CLI: `bd update issue-42 --notes "COMPLETED: User authentication..."`)

**Bad note (insufficient for recovery):**
```json
{
  "issue_id": "issue-42",
  "notes": "Working on auth feature. Made some progress. More to do later."
}
```
...
**After compaction**: Use `mcp__plugin_beads_beads__show` to reconstruct full context needed to continue work.

### Before/After Compaction: The Dramatic Difference

This example shows what survives compaction and what doesn't - illustrating why good bd notes are critical.

**BEFORE COMPACTION - Rich Conversation Context:**

```
User: "Hey, can you help me add OAuth to the auth system?"

Claude: "Sure! Let me check what's already there."
[reads auth code]

Claude: "I see you have basic JWT auth. For OAuth, we'll need to:
1. Set up credentials
2. Implement authorization flow
3. Add token refresh
Should I start?"

User: "Yes, but use RS256 not HS256 - security review requirement"

Claude: "Got it, RS256 for key rotation capability. Starting..."
[implementation work, multiple messages]

User: "Wait, the access token expiry - use 1 hour not 15 minutes"

Claude: "Changing to 1hr access tokens..."

User: "Actually, make refresh tokens 7 days, not 30"

Claude: "Updated to 7-day refresh tokens per your preference."

[20 more messages of back-and-forth implementation]

Claude: "OAuth is working! Tests passing. Let me update the beads notes."

bd update auth-42 --notes "COMPLETED: OAuth implementation with Google/GitHub
providers. JWT tokens using RS256 algorithm (per security review requirement).
Access tokens: 1hr expiry. Refresh tokens: 7-day expiry (user preference).
Implemented token rotation on refresh. IN PROGRESS: None. NEXT: None - ready to
close. KEY DECISION: RS256 chosen over HS256 to enable key rotation without
invalidating all tokens. User specified token expiry times after initial
discussion."
```

**⚠️ COMPACTION EVENT OCCURS**

All conversation history deleted. TodoWrite lists deleted. Only bd state survives.

**AFTER COMPACTION - New Session, Zero Conversation Memory:**

New Claude agent with no memory of previous session sees:

```bash
$ bd list --status in_progress
# Returns: auth-42 "OAuth implementation"
...
Notes:
COMPLETED: OAuth implementation with Google/GitHub providers. JWT tokens using
RS256 algorithm (per security review requirement). Access tokens: 1hr expiry.
Refresh tokens: 7-day expiry (user preference). Implemented token rotation on
refresh.

IN PROGRESS: None

NEXT: None - ready to close

KEY DECISION: RS256 chosen over HS256 to enable key rotation without
invalidating all tokens. User specified token expiry times after initial
discussion.
```
...
```
Claude (new session): "I see auth-42 'OAuth implementation' is in_progress.

From the notes:
✓ Completed: OAuth with Google/GitHub providers
✓ Completed: RS256 JWT tokens (1hr access, 7-day refresh)
✓ Completed: Token rotation implemented
✓ Next: Ready to close

The implementation looks complete with tests passing. Should I close this issue,
or is there more work needed?"

User: "Close it, it's done"

Claude: bd close auth-42 --reason "OAuth implementation complete with RS256
tokens and rotation"
```

**What Survived Compaction:**
- ✅ All technical decisions (RS256, token expiries)
- ✅ User preferences (7-day refresh, 1hr access)
- ✅ Rationale for choices (why RS256 over HS256)
- ✅ Current state (complete and ready to close)
- ✅ Work history (what was implemented)

**What Was Lost:**
- ❌ Conversation flow and back-and-forth
- ❌ Individual messages
- ❌ TodoWrite progress lists
- ❌ Exact timing of decisions

**Key Insight:** Well-written notes preserve all *essential* context even when conversation history is completely deleted. The new session can immediately understand what was done, why, and what comes next.

**Bad notes would have looked like:**
```
Notes: Working on OAuth. Made progress. Some issues with tokens but fixed them.
```

After compaction, this would be useless - no way to know what was decided, what's working, or what's left.
...
```
With MCP (local environment):
- [ ] Check for .beads/ directory
- [ ] If exists: use mcp__plugin_beads_beads__ready
- [ ] Report status to user
- [ ] Get user input on what to work on
- [ ] Use mcp__plugin_beads_beads__show for issue details
- [ ] Use mcp__plugin_beads_beads__update to set status: in_progress
- [ ] Begin work

CLI fallback (web environment):
- [ ] Check for .beads/ directory
- [ ] If exists: bd ready --json
- [ ] Report status to user, bd show <id>, bd update <id> --status in_progress
```
