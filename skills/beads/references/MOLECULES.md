...
**Phase transitions:**
- `pour`: Solid (proto) → Liquid (mol)  *(v0.36.0: `spawn` removed, use `pour`)*
...
## Creating Molecules (v0.36.0+)

**Note:** `bd mol spawn` was removed in v0.36.0. Use `bd pour` for persistent mols and `bd wisp create` for ephemeral wisps.

```bash
bd pour mol-feature                        # Create persistent mol
bd pour mol-release --var version=2.0      # With variable substitution
bd wisp create mol-patrol                  # Create ephemeral wisp
bd wisp create mol-check --var target=db   # Wisp with variables
```
...
1. Pours the molecule (persistent)
...
```bash
bd pour mol-feature --attach mol-testing --var name=auth
# Pours mol-feature, then pours mol-testing and bonds them
```
...
```bash
bd pour mol-deploy --attach mol-rollback --attach-type conditional
```
...
bd wisp create mol-check --var target=db     # With variables
```
...
Formulas are declarative workflow definitions in TOML format.

### Basic Usage

```bash
bd cook --list                          # List available formulas
bd cook beads-release --var version=2.0 # Execute formula
```

### Formula Structure

Formulas live in `.beads/formulas/` and define multi-step workflows with:
- Steps (issues to create)
- Dependencies between steps
- Variables for parameterization
- Gates for async coordination

**For detailed formula authoring:** See bd documentation on formula TOML syntax.

---

## Gates (v0.36.0+)

Gates are async coordination points that block until conditions are met.

### Gate Types

| Type | Syntax | Waits for |
|------|--------|-----------|
| Timer | `timer:2h` | Time duration |
| GitHub PR | `gh:pr:merged` | PR to be merged |
| GitHub Run | `gh:run:success` | Workflow to complete |
| Mail | `mail:<address>` | Email response |
| Human | (no await) | Manual approval |
...
# Evaluate gate (check if conditions met)
bd gate eval .claude-xyz

# Human approval
bd gate approve .claude-xyz
```

---

## Activity Monitoring (v0.35.0+)

Monitor molecule execution in real-time:

```bash
bd activity              # Real-time state feed
bd mol next              # Navigate to next step
bd mol prev              # Navigate to previous step
...
| `bd pour <proto>` | Create persistent mol from proto |
| `bd wisp create <proto>` | Create ephemeral wisp |
| `bd mol run <proto>` | Pour + assign + pin (durable execution) |
...
| `bd cook <formula>` | Execute declarative workflow (v0.36.0+) |
| `bd gate create` | Create async coordination point (v0.36.0+) |
| `bd gate eval` | Evaluate gate conditions (v0.36.0+) |
| `bd gate approve` | Human approval for gate (v0.36.0+) |
| `bd activity` | Real-time molecule monitoring (v0.35.0+) |
