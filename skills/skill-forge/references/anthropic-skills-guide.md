...
> Source: Anthropic PDF guide (January 2026). Converted to markdown for reference by skill-forge.
...
> **Pro Tip: Iterate on a single task before expanding.** We've found that the most effective skill creators iterate on a single challenging task until Claude succeeds, then extract the winning approach into a skill. This leverages Claude's in-context learning and provides faster signal than broad testing.
...
**Iterative improvement:** After encountering edge cases or failures, bring those examples back to skill-creator.
...
- **Solution:** Add more detail and nuance to the description — keywords for technical terms
...
Anthropic has published Agent Skills as an open standard. Like MCP, skills should be portable across tools and platforms — the same skill should work whether you're using Claude or other AI platforms. Authors can note platform-specific capabilities in the skill's `compatibility` field.

### Using skills via API

For programmatic use cases — building applications, agents, or automated workflows that leverage skills — the API provides direct control.
...
1. **Host on GitHub** — Public repo, clear README, example usage and screenshots
2. **Document in Your MCP Repo** — Link to skills, explain the combined value, provide quick-start guide
...
- Error "Could not find SKILL.md in uploaded folder" — Rename to exactly `SKILL.md` (case-sensitive)
- Error "Invalid frontmatter" — Check YAML formatting: `---` delimiters required, close all quotes
- Error "Invalid skill name" — Must be kebab-case, no spaces or capitals

**Skill doesn't trigger**

Revise your description field. Quick checklist:
- Is it too generic? ("Helps with projects" won't work)
- Does it include trigger phrases users would actually say?
- Does it mention relevant file types if applicable?

Debugging approach: Ask Claude "When would you use the [skill name] skill?" — it will quote the description back.
...
2. Check authentication (API keys valid, proper scopes, OAuth tokens refreshed)
...
3. **Ambiguous language** — Be specific: `"CRITICAL: Before calling create_project, verify: project name is non-empty, at least one team member assigned, start date is not in the past"`
...
**Support:** Community forums at the Claude Developers Discord. Bug reports: GitHub Issues at anthropics/skills/issues.
...
Forbidden: XML angle brackets (`<` `>`), code execution in YAML (uses safe YAML parsing), skills named with "claude" or "anthropic" prefix (reserved).
